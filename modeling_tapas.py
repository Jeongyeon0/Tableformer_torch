...
class TapasSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.table_bias = torch.zeros([13, self.num_attention_heads])
    ...
    def compute_relative_attention_scores(self, relative_bias_table,  relation_att_ids):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        Shape = list(relation_att_ids.shape)
        Shape.extend([relative_bias_table.shape[1]])
        out = torch.zeros(Shape).to(device)
        out = out.squeeze(dim=1)
        for batch in range(Shape[0]):
            for query_len in range(Shape[1]):
                for num_heads in range(Shape[2]):
                    out[batch][query_len] = relative_bias_table[relation_att_ids[batch][query_len][num_heads]]
        relative_att_scores = out.permute(0,3,1,2)
    ...
    def forward(
        ...
        if relative_attention_ids is not None:
            relative_attn_scores = self.compute_relative_attention_scores(self.table_bias, relative_attention_ids)
        if self.is_decoder:
        ...
        if relative_attention_ids is not None:
            attention_scores += relative_attn_scores
...
class TapasLayer(nn.Module):
    ...
    def forward(
        ...
        self_attention_outputs = self.attention(
            ...
            relative_attention_ids=relative_attention_ids,
...
class TapasModel(TapasPreTrainedModel):
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            input_ids=input_ids, position_ids=None, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds
        )
